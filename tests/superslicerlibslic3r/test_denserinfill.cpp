...
const ExtrusionEntitiesPtr& get_island(const ExtrusionEntityCollection &coll, int island) {
...
SCENARIO("denser infills: ")
{
    ...
    GIVEN("center hole")
    {
        WHEN("dense infill to enlarged") {
            ...
            //        std::cout << "    - fills= " << get_island(object.layers()[lidx]->regions()[ridx]->fills(),0).size() << "\n";
            ...
            THEN("correct number of fills") {
                ...
                REQUIRE(get_island(object.layers()[21]->regions()[0]->fills(),0).size() == 2); //sparse + dense
                REQUIRE(object.layers()[21]->regions()[0]->fill_surfaces().surfaces.size() == 2);
                REQUIRE(object.layers()[21]->regions()[0]->fill_surfaces().surfaces[0].surface_type == (SurfaceType::stDensSparse | SurfaceType::stPosInternal | stModBridge));
                ...
                Surface* srfSparse = &object.layers()[21]->regions()[0]->set_fill_surfaces().surfaces[0];
                Surface* srfDense = &object.layers()[21]->regions()[0]->set_fill_surfaces().surfaces[1];
                if (srfSparse->maxNbSolidLayersOnTop == 1) {
                    srfSparse = srfDense;
                    srfDense = &object.layers()[21]->regions()[0]->set_fill_surfaces().surfaces[0];
                }
                ...
                REQUIRE(get_island(object.layers()[22]->regions()[0]->fills(),0).size() == 2); //sparse + solid-bridge
                ...
                REQUIRE(get_island(object.layers()[23]->regions()[0]->fills(),0).size() == 2); //sparse + solid
                REQUIRE(get_island(object.layers()[24]->regions()[0]->fills(),0).size() == 3); //sparse + solid-top + solid-top (over perimeters)
                REQUIRE(get_island(object.layers()[25]->regions()[0]->fills(),0).size() == 1); //sparse
            }
        }
    }
    ...
    GIVEN("side hole")
    {
        WHEN("dense infill to auto") {
            ...
            THEN("correct number of fills") {
                ...
                REQUIRE(get_island(object.layers()[24]->regions()[0]->fills(),0).size() == 3); //sparse + solid-top + solid-top (over perimeters)
                REQUIRE(get_island(object.layers()[25]->regions()[0]->fills(),0).size() == 1); //sparse
                REQUIRE(get_island(object.layers()[45]->regions()[0]->fills(),0).size() == 1); //sparse
                ...
                REQUIRE(get_island(object.layers()[46]->regions()[0]->fills(),0).size() == 1); //dense
                REQUIRE(get_island(object.layers()[46]->regions()[0]->fills(),0).size() == 1);
                ...
                REQUIRE(get_island(object.layers()[47]->regions()[0]->fills(),0).size() == 1); //solid-bridge
                ...
                REQUIRE(get_island(object.layers()[48]->regions()[0]->fills(),0).size() == 1); //solid
                ...
                REQUIRE(get_island(object.layers()[49]->regions()[0]->fills(),0).size() == 1); //top
