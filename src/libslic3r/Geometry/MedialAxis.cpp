...
MedialAxis::polyline_from_voronoi(const ExPolygon& voronoi_polygon, ThickPolylines* polylines)
{
    ...
    while (!edges.empty()) {
        ...
        if (thickness[edge].first > this->m_max_width * 1.001) {
            //std::cerr << "Error, edge.first has a thickness of " << unscaled(this->thickness[edge].first) << " > " << unscaled(this->m_max_width) << "\n";
            //(void)this->edges.erase(edge);
            //(void)this->edges.erase(edge->twin());
            //continue;
        }
        if (thickness[edge].second > this->m_max_width * 1.001) {
            //std::cerr << "Error, edge.second has a thickness of " << unscaled(this->thickness[edge].second) << " > " << unscaled(this->m_max_width) << "\n";
            //(void)this->edges.erase(edge);
            //(void)this->edges.erase(edge->twin());
            //continue;
        }
    ...
    while (changes) {
        ...
        for (size_t i = 0; i < pp.size(); ++i) {
            ...
            if ((polyline.endpoints.first || polyline.endpoints.second)) {
                coordf_t local_max_width = this->m_max_width / 2;
...
MedialAxis::simplify_polygon_frontier()
{
    ...
    this->m_expolygon.contour.remove_collinear(SCALED_EPSILON);
    for (Polygon& hole : this->m_expolygon.holes)
