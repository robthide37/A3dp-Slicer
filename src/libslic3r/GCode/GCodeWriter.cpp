...
void GCodeWriter::apply_print_config(const PrintConfig &print_config)
{
    ...
    m_formatter = GCodeFormatter(this->m_config.gcode_precision_xyz.value, this->m_config.gcode_precision_e.value);
}
    ...
    if (this->multiple_extruders) {
        if (FLAVOR_IS(gcfKlipper)) {
            ...
            if (tool_id > 0 && tool_id < this->m_config.tool_name.size() && !this->m_config.tool_name.get_at(tool_id).empty()
                ...
                && this->m_config.tool_name.get_at(tool_id)[0] != static_cast<char>(('0' + tool_id))) {
...
std::string GCodeWriter::travel_to_xyz(const Vec3d &point, const bool is_lift, const double speed, const std::string_view comment)
{
    ...
    if (this->m_config.travel_speed_z.value > 0) {
        const double dist_xyz        = (m_pos - point).norm();
        double       duration_xyz = dist_xyz / travel_speed;
        double       min_duration_z  = std::abs(m_pos.z() - point.z()) / this->m_config.travel_speed_z.value;
    ...
    bool has_z = w.emit_z(point.z() + this->gcode_config().z_offset.value, m_pos_str_z);
    if (!has_x_y && !has_z) {
...
std::string GCodeWriter::get_travel_to_z_gcode(const double z, const std::string_view comment)
{
    ...
    bool has_z = w.emit_z(z + this->gcode_config().z_offset.value, m_pos_str_z);
    if (!has_z) {
        return "";
    }
    w.emit_f(speed * 60.0);
