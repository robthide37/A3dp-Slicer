...
public:
    ...
    const GCodeFormatter& get_default_gcode_formatter(){ return 
...
//    static constexpr const int E_EXPORT_DIGITS    = 9;
    ...
    Vec2d                                  quantize(const Vec2d &pt) const
        { return { quantize(pt.x(), XYZF_EXPORT_DIGITS), quantize(pt.y(), m_gcode_precision_xyz) }; }
    Vec3d                                  quantize(const Vec3d &pt) const
        { return { quantize(pt.x(), XYZF_EXPORT_DIGITS), quantize(pt.y(), m_gcode_precision_xyz), quantize(pt.z(), m_gcode_precision_xyz) }; }
