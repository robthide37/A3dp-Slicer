...
//    Point                 centroid;
    ...
    void                  add_ironing_range(const LayerExtrusionRange &new_ironing_range) {
        // Compress ranges.
        if (! this->ironings.empty() && this->fills.back().region() == new_fill_range.region() && *this->fills.back().end() == *new_fill_range.begin())
            this->ironings.back() = { new_ironing_range.region(), { *this->fills.back().begin(), *new_fill_range.end() } };
        else
            this->ironings.push_back(new_fill_range);
    }
};
