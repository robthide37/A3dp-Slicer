///|/ Copyright (c) Prusa Research 2022 - 2023 Oleksandra Iushchenko @YuSanka, David Kocík @kocikdav, Vojtěch Bubník
///@bubnikv, Enrico Turri @enricoturri1966, Lukáš Matěna @lukasmatena
...
bool AppUpdater::priv::http_get_file(const std::string &url,
    ...
    Http::get(url)
        .on_progress([&, progress_fn](Http::Progress progress, bool &cancel) {
            ...
            if (cancel) {
                // Lets keep error_message empty here - if there is need to show error dialog, the message will be
                // probably shown by whatever caused the cancel.
        ...
        if (section_name == "common") {
            ...
            for (const auto &data : section.second) {
                ...
                if (data.first == "release") {
                    ...
                    SLIC3R_APP_NAME, version); wxCommandEvent* evt = new wxCommandEvent(EVT_SLIC3R_VERSION_ONLINE);
                    evt->SetString(GUI::from_u8(version));
                    GUI::wxGetApp().QueueEvent(evt);
                    */
