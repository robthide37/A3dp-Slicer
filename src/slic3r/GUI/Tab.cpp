    ...
    if (type() == Preset::TYPE_PRINTER) {
        ...
        if (this->get_printer_technology() == ptFFF) {
            ...
            if (tab->m_initial_laser_count != tab->m_laser_count)
                dirty_options.emplace(OptionKeyIdx::scalar("laser_count"), PresetCollection::DIRTY_VECTOR_CHANGE_SIZE);
            if (tab->m_sys_laser_count != tab->m_laser_count)
                nonsys_options.emplace(OptionKeyIdx::scalar("laser_count"), PresetCollection::DIRTY_VECTOR_CHANGE_SIZE);
        }
    }
...
void TabPrinter::init_options_list()
{
    ...
    if (m_printer_technology == ptFFF) {
            ...
            m_options_list.emplace(OptionKeyIdx::scalar("laser_count"), m_opt_status_value);
    }
    } else {
        Tab::init_options_list();
        ...
        m_options_list[OptionKeyIdx::scalar("laser_count")] = m_opt_status_value;
    }
}
    ...
    while (cur_item) {
        ...
        for (auto page : m_pages)
        {
            ...
            if (page->title() == "General") {
                std::initializer_list<const char*> optional_keys{ "extruders_count", "milling_count", "laser_count", "bed_shape" };
...
void Tab::on_roll_back_value(const bool to_sys /*= true*/)
{
    ...
    for (auto group : m_active_page->m_optgroups) {
        ...
        if (group->has_option_def(OptionKeyIdx::scalar("milling_count"))) {
            assert(m_options_list.find(OptionKeyIdx::scalar("milling_count")) != m_options_list.end());
            if ((m_options_list[OptionKeyIdx::scalar("milling_count")] & os) == 0)
                to_sys ? group->back_to_sys_value(OptionKeyIdx::scalar("milling_count")) :
                         group->back_to_initial_value(OptionKeyIdx::scalar("milling_count"));
        }
        if (group->has_option_def(OptionKeyIdx::scalar("laser_count"))) {
            assert(m_options_list.find(OptionKeyIdx::scalar("laser_count")) != m_options_list.end());
            if ((m_options_list[OptionKeyIdx::scalar("laser_count")] & os) == 0)
                to_sys ? group->back_to_sys_value(OptionKeyIdx::scalar("laser_count")) :
                         group->back_to_initial_value(OptionKeyIdx::scalar("laser_count"));
        }
...
//                layout_page(current_page);
            ...
            for (int i = 1; i < params.size() - 1; i++) {
                ...
                else if (params[i].find("sidetext_width$") != std::string::npos)
                {
                    ...
                    current_group->m_on_change = set_or_add(current_group->m_on_change,
                        ...
                        // Thus, there should be boost::any_cast<int> !
                        // Otherwise, boost::any_cast<size_t> causes an "unhandled unknown exception"
                        if (opt_key_idx.key == "laser_count") {
                            size_t laser_count = size_t(boost::any_cast<int>(current_group_sh->get_value(OptionKeyIdx::scalar("laser_count"))));
        ...
        else if (boost::starts_with(full_line, "setting"))
        {
            ...
            if (boost::starts_with(full_line, "recommended_thin_wall_thickness_description")) {
                ...
                ConfigOptionDef def{"laser_count", coInt};
                def.set_default_value(new ConfigOptionInt(0));
...
void TabPrinter::lasers_count_changed(size_t laser_count)
{
    ...
    if (is_count_changed) {
        ...
        //propagate change
        on_value_change(OptionKeyIdx::scalar("laser_count"), laser_count);
...
void TabPrinter::reload_config()
{
    ...
    if (m_active_page && m_active_page->get_field("laser_count"))
        m_active_page->set_value(OptionKeyIdx::scalar("laser_count"), int(m_laser_count), true);
}
