...
IntPoint Centroid(const Path& poly, double area)
{
    ...
    return IntPoint(cInt(x_temp / (6 * area)), cInt(y_temp / (6 * area)));
}
//------------------------------------------------------------------------------
  ...
  for(size_t i = 1; i <= cnt; ++i)
  {
    ...
    if ((ip.y() < pt.y()) != (ipNext.y() < pt.y()))
    {
      ...
      } else
      {
        if (ipNext.x() > pt.x())
        {
          double d = CrossProductType(ip.x() - pt.x()) * CrossProductType(ipNext.y() - pt.y()) - CrossProductType(ipNext.x() - pt.x()) * CrossProductType(ip.y() - pt.y());
          if (!d) return -1;
          if ((d > 0) == (ipNext.y() > ip.y())) result = 1 - result;
        }
      }
  ...
  do
  {
    ...
    if ((op->Pt.y() < pt.y()) != (op->Next->Pt.y() < pt.y()))
    {
      if (op->Pt.x() >= pt.x())
      {
        ...
        {
          double d = CrossProductType(op->Pt.x() - pt.x()) * CrossProductType(op->Next->Pt.y() - pt.y()) - CrossProductType(op->Next->Pt.x() - pt.x()) * CrossProductType(op->Pt.y() - pt.y());
          if (!d) return -1;
